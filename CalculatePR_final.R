...
Sample.Info <- read_csv(file = here("Data_Raw",
                                    "Coral",
                                    "Metabolism",
                                    "PR_final",
                                    "Metadata",
...
for (i in 1: length(file.names.full)) {
  ...
  pdf(paste0(here("Data_Raw", "Coral", "Metabolism", "PR_final", "Output", "RespoOutput"),"/", rename,"thinning.pdf"))
...
Respo.R_Normalized <- Respo.R %>%
  ...
  mutate(umol.sec.corr = umol.sec - blank.rate, # subtract the blank rates from the raw rates
         mmol.gram.hr = 0.001*(umol.sec.corr*3600)/totalSA_cm2)  %>% # convert to mmol g hr-1
  filter(BLANK ==0) %>% # remove all the blank data
  select(Date, PlateID, CowTagID, FragmentID, Top_Bottom,Light_Dark, Site, totalSA_cm2, mmol.gram.hr) #keep only what we need
...
write_csv(Respo.R_Normalized,here("Data",
                                  "Coral",
                                  "Metabolism",
                                  "PR_final_normalized_2022_06_28.csv"))# export all the uptake rates
View(Respo.R)
